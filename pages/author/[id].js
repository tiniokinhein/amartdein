...
const AuthorDetail = ({ itemId }) => {
    ...
    React.useEffect(() => {
        ...
        axios 
        .get(NEWS + `?author=${itemId.id}&per_page=100&_embed=1`)
        .then(res => {
            setItems(res.data)
        })
        .catch(err => console.log(err.message))
    ...
    const combine = t('a.d', {
        d_en: p ? p.acf.full_name_en : null,
        d_mon: p ? p.acf.full_name_mon : null
    })
    ...
    const headList = p ? (
        <div key={p.id} className="py-5 d-flex align-items-center">
            <div className="pe-3">
                <img 
                    src={p.acf.image.url}
                    alt=""
    ...
    const otherAuthorList = users && users.filter(fr => (p ? p.id : null) !== fr.id && fr.id !== 1).length ? (
        <div className="pb-5">
            <h4 
